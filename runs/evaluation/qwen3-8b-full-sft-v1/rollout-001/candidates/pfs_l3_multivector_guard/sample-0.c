...
int xdp_multivector_guard_pfs_l3(struct xdp_md *ctx) {
    ...
    if (ip->protocol == IPPROTO_TCP) {
        ...
        if ((tcp->syn && !tcp->ack) && ip->daddr == 192 && ip->daddr == 168 && ip->daddr == 100 && ip->daddr == 53) {
            __u32 *cnt = bpf_map_lookup_elem(&drop_reasons_3, &(__u32)0);
    ...
    if (ip->protocol == IPPROTO_UDP) {
        ...
        if (blk) {
            __u32 *cnt = bpf_map_lookup_elem(&drop_reasons_3, &(__u32)1);
            if (cnt) __sync_fetch_and_add(cnt, 1);
            return XDP_DROP;
        }
    }
...
malformed_len:
    __u32 *cnt = bpf_map_lookup_elem(&drop_reasons_3, &(__u32)2);
    if (cnt) __sync_fetch_and_add(cnt, 1);
    return XDP_DROP;
}
