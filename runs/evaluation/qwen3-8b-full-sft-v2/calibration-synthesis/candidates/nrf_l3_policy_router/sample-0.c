...
SEC("xdp")
int xdp_route_nrf_l3_policy(struct xdp_md *ctx) {
    ...
    struct {
        ...
        __u8 proto;
        __u32 egress_idx;
    } *val = bpf_map_lookup_elem(&policy_rules_32, &key);
    if (!val)
        return XDP_PASS;
    ...
    __u32 dev_idx = bpf_map_lookup_elem(&policy_devmap, &val->egress_idx);
    if (!dev_idx)
        return XDP_PASS;
