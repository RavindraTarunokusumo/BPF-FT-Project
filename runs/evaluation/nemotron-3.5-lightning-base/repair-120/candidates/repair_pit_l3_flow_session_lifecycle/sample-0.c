...
/* Minimal flow key: 5-tuple packed into 16 bytes to keep map value small */
struct flow_key {
    __u32 saddr;
    __u32 daddr;
    __u16 sport;
...
/* Compact lifecycle record: 32 bytes total – well under the 512‑byte stack limit */
...
int xdp_telemetry(struct xdp_md *ctx)
{
    ...
    /* Look up or create the per‑flow record */
