...
int xdp_telemetry(struct xdp_md *ctx) {
    ...
    if (ip->ihl > 5) {
        ...
        char *opt = (char *)(ip + 1);
        __u32 i = 0;
        /* Unrolled loop: statically bounded to 10 iterations (max 40 bytes). */
        for (i = 0; i < opt_len && i < 10; i += 1) {
#pragma unroll max 10
            /* Ensure at least one byte (the option type) is present. */
