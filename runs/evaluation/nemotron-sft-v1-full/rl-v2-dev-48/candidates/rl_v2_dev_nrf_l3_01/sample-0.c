...
struct {
    ...
    __key type struct route_lpm_key;
    __value type unsigned char[ETH_ALEN];
    __flags BPF_F_NO_PREALLOC;
} dev_route_table_1 SEC(".maps");

SEC("xdp")
int xdp_route_rl_v2_dev_nrf_l3_01(struct xdp_md *ctx) {
    ...
    unsigned char *gw_mac = bpf_map_lookup_elem(&dev_route_table_1, &key);
    if (gw_mac) {
