...
struct {
    ...
    __type(key, __u32);
    __type(value, struct vlan_telemetry_entry);
...
int xdp_telemetry_pit_l3_vlan_dualstack_telemetry(struct xdp_md *ctx) {
    ...
    if (eth->h_proto == bpf_htons(ETH_P_IP))
        ...
        if (vlan->h_vlan_encapsulated_proto == bpf_htons(ETH_P_IP))
            key = 1;
        else if (vlan->h_vlan_encapsulated_proto == bpf_htons(ETH_P_IPV6))
            key = 2;
    }

    pkt_len = (__u64)data_end - (__u64)data;
    struct vlan_telemetry_entry *e = bpf_map_lookup_elem(&vlan_telemetry_map, &key);
    if (e) {
        __sync_fetch_and_add(&e->packets, 1);
        __sync_fetch_and_add(&e->bytes, pkt_len);
    }
