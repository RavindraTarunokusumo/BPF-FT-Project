#!/usr/bin/env python3
"""
Automatically scans data/validation/<category>/<level>/ and sets gold_candidate_id
in every task.json across all categories and difficulty levels.
"""
...
def main() -> None:
    ...
    for cat_dir in sorted(INBOX_DIR.iterdir()):
        ...
        for lvl_dir in sorted(cat_dir.iterdir()):
            ...
            val_lvl_dir = VAL_DIR / cat_dir.name / lvl_dir.name
            ...
            for task_dir in sorted(lvl_dir.iterdir()):
                ...
                for cand_file in sorted(task_dir.glob("c*.c"), reverse=True):
                    ...
                    val_file = val_lvl_dir / f"{cand_id}.json"
                    if val_file.exists():
                        try:
                ...
                if gold_cand_id:
                    ...
                    updated_tasks += 1
                    print(f"[+] {cat_dir.name}/{lvl_dir.name}/{task_id} -> gold: {gold_cand_id}")
                else:
                    print(f"[-] WARNING: No passing candidate for {cat_dir.name}/{lvl_dir.name}/{task_id}")
