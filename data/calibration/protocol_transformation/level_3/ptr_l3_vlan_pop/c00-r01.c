...
int xdp_vlan_pop(struct xdp_md *ctx) {
    ...
    // Check VLAN header bounds (4 bytes after Ethernet header)
    if (data + sizeof(struct ethhdr) + 4 > data_end)
        return XDP_PASS;

    // Check inner EtherType is not VLAN (double-tagged)
    __be16 inner_ethertype = *( (__be16 *)((char *)data + sizeof(struct ethhdr) + 2));
    if (inner_ethertype == bpf_htons(ETH_P_8021Q)) {
        return XDP_PASS;
    }

    // Check inner EtherType is within bounds
    if (data + sizeof(struct ethhdr) + 2 > data_end)
        return XDP_PASS;
